...
See `.github/actions/select-copilot-pat/README.md` for full documentation and `.github/workflows/conformance-tier-audit.md` for a working example.

### How it works

When **creating** or **updating** an agentic workflow, always include the following three frontmatter blocks. These elements are **not supported through imports**, so they must be copied into every workflow's frontmatter. See `.github/workflows/conformance-tier-audit.md` for a working example.
...
- Do **not** specify `engine: copilot` as a simple string — use the object form so the `env:` override can be included.
- Different workflows can use different pool name prefixes if desired.
